...
(() => {
  ...
  function placeSelectedRelicManually(index){
    ...
    if(!pl){ relicMessage.textContent="先に右側の候補から配置するレリックを選択してください。"; return false; }
    if(pl.enabled===false){ relicMessage.textContent="このレリックはチェックOFFです。先に✓を付けてください。"; return false; }
    if(!relicPatternForPlacement(pl).length){ relicMessage.textContent="このレリックは形状未登録です。先に形状を登録してください。"; return false; }
    const tapX=index%7, tapY=Math.floor(index/7);
    const rotated=rotateCells(relicPatternForPlacement(pl),pl.rotation);
    // タップしたマスを「形状のどれか1マス」として扱い、成立する配置位置を探す。
    // これにより形状の基準点が右/下へ寄っていても盤面上で手動配置できる。
    let candidate=null;
    const anchors=rotated.length?rotated:[[0,0]];
    for(const [ax,ay] of anchors){
      const test={...pl,x:tapX-ax,y:tapY-ay};
      if(canPlaceRelic(test,pl.uid)){ candidate=test; break; }
    }
    if(!candidate){
      relicMessage.textContent="この位置では配置できません。別のマスをタップしてください。";
      return false;
    }
  ...
  function updateManualPlacementUi(){
    ...
    if(relicManualHint){
      relicManualHint.textContent=relicManualPlacementMode
        ?"手動配置ON：候補レリックを選択 → 盤面の置きたい位置をタップ。配置済みレリックのマスをタップするとそのレリックを選択できます。"
        ...
        cell.addEventListener("click",()=>{
          ...
          if(relicManualPlacementMode){
            ...
            if(hit && hit.uid!==state.selectedRelicUid){
              ...
              relicMessage.textContent=`${item?.["名前"]||"レリック"}を選択しました。空きマスをタップすると移動できます。`;
              renderBuild();
              return;
            }
...
function collectTotalData() {
  ...
  document.getElementById("partialResetButton")?.addEventListener("click",()=>{
    const select=document.getElementById("partialResetSelect");
    const kind=String(select?.value||"");
    if(!kind){
      const msg=document.getElementById("shareMessage");
      if(msg) msg.textContent="リセットする項目を選んでください。";
      return;
    }
    partialReset(kind);
