...
(() => {
  ...
  function relicManualAnchorCell(placement){
    const cells=rotateCells(relicPatternForPlacement(placement),placement.rotation)
  ...
  function renderRelicBoard() {
    ...
    relicPlacementList.querySelectorAll("[data-relic-manual]").forEach(button=>{
      button.addEventListener("click",event=>{
        ...
        updateManualPlacementUi();
        const item=context.itemsById.get(String(pl.itemId));
        relicMessage.textContent=`${item?.["名前"]||"レリック"}を手動配置対象にしました。盤面の置きたいマスをタップしてください。`;
        renderRelicBoard();
      });
    });
