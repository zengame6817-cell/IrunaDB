...
(() => {
  ...
  function relicManualCandidateAt(index, placement=null, rotationOverride=null){
    ...
    if(!pl || pl.enabled===false || !relicPatternForPlacement(pl).length) return null;
    ...
    const tapX=index%7,tapY=Math.floor(index/7);
    const rotated=rotateCells(relicPatternForPlacement(pl),rotation);
    if(!rotated.length) return null;

    // タップしたマスを形状内の「どのマス」に合わせるか全候補を試す。
    // 置ける候補の中から、現在位置に最も近いものを採用する。
    const attempts=[];
    for(const [ax,ay] of rotated){
      const candidate={...pl,x:tapX-ax,y:tapY-ay,rotation};
      const check=relicPlacementCheck(candidate,pl.uid);
      const dist=(Number(pl.x)>=0&&Number(pl.y)>=0)
        ? Math.abs(candidate.x-Number(pl.x))+Math.abs(candidate.y-Number(pl.y))
        : Math.abs(candidate.x-3)+Math.abs(candidate.y-3);
      attempts.push({candidate,valid:check.valid,reason:check.reason,cells:absoluteRelicCells(candidate),anchor:[tapX,tapY],dist});
    }
    const valid=attempts.filter(v=>v.valid).sort((a,b)=>a.dist-b.dist)[0];
    if(valid) return valid;

    // NGプレビューも、盤面内に見えるマスが最も多い候補を表示する。
    attempts.forEach(v=>{
      v.visible=v.cells.filter(([x,y])=>x>=0&&x<7&&y>=0&&y<7).length;
    });
    return attempts.sort((a,b)=>(b.visible-a.visible)||(a.dist-b.dist))[0] || null;
  }
  ...
  function selectedRelicActualAnchorIndex(){
    ...
    const cells=absoluteRelicCells(pl).filter(([x,y])=>x>=0&&x<7&&y>=0&&y<7)
  ...
  function confirmManualPlacement(){
    ...
    relicManualDraftRotation=Number(pl.rotation)||0;
    relicManualPreviewIndex=selectedRelicActualAnchorIndex();
  ...
  function updateManualPlacementUi(){
    ...
    if(relicManualHint){
      relicManualHint.textContent=relicManualPlacementMode
        ?"手動配置ON：盤面をタップして位置をプレビュー → 90°回転で向きを変更 → 「変更確定」で保存します。"
  ...
  function renderRelicV3Board(){
    ...
    [...relicV3Board.children].forEach((cell,i)=>{
      ...
      if(b.blocked[i]) cell.classList.add("is-blocked");
  ...
  function renderRelicBoard() {
    ...
    relicPlacementList.querySelectorAll("[data-relic-manual]").forEach(button=>{
      button.addEventListener("click",event=>{
        ...
        beginManualDraft(pl);
        const item=context.itemsById.get(String(pl.itemId));
        relicMessage.textContent=`${item?.["名前"]||"レリック"}を手動配置対象にしました。◎の支点を置きたい盤面マスに合わせてください。1回目でプレビュー、2回目で確定します。`;
        renderRelicBoard();
      });
    });
