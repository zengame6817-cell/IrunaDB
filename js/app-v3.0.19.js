...
(() => {
  ...
  function migrateLegacyRelicBlockedCellsV3016(){
    const key="irunadb.relic.blockedMigration.v3.0.19";
      ...
      relicV3Board.querySelectorAll("[data-index]").forEach(cell=>{
        ...
        cell.addEventListener("click",()=>{
          ...
          if(relicManualPlacementMode){
            // 同じ支点をもう一度クリックすると固定解除し、PCでは再びカーソル追従に戻る。
            if(relicManualPreviewFrozen && relicManualPreviewIndex===i){
              relicManualPreviewFrozen=false;
              updateManualPlacementUi();renderRelicV3Board();
              relicMessage.textContent="一時固定を解除しました。PCでは再びカーソルにピースが追従します。";
              return;
            }
            const preview=setManualDraftAt(i);
            ...
            updateManualPlacementUi();renderRelicV3Board();
            relicMessage.textContent=preview?.valid
              ?"この位置に一時固定しました。同じマスをもう一度クリックすると固定解除できます。必要なら90°回転 → 「変更確定」。"
              :`この位置では配置不可：${preview?.reason||"不明"}。同じマスをもう一度クリックで固定解除できます。`;
            return;
          }
          ...
          if(occupiedHit && !relicBlockEditModeEnabled){
            ...
            relicMessage.textContent=`${item?.["名前"]||"レリック"}を直接編集します。現在位置で一時固定中。別マスをクリックして移動、90°回転、変更確定ができます。`;
            renderRelicV3Board();
            return;
          }
  ...
  function transferRgRelicsToBuild(){
    ...
    if(buildRelicMessage){
      buildRelicMessage.textContent=ids.length
        ? `RGのチェックON ${ids.length}個をビルドへ転送しました。`
        : "RGにチェックONのレリックがありません。";
    }
    syncUrl(false);
    renderBuild();
    setView("build");
