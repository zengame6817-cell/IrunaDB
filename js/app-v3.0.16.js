...
(() => {
  ...
  function migrateLegacyRelicBlockedCellsV3016(){
    const key="irunadb.relic.blockedMigration.v3.0.16";
  ...
  function relicManualCandidateAt(index, placement=null, rotationOverride=null){
    ...
    const x=index%7,y=Math.floor(index/7);
    const candidate={...pl,x,y,rotation};
    const check=relicPlacementCheck(candidate,pl.uid);
    return {candidate,valid:check.valid,reason:check.reason,cells:absoluteRelicCells(candidate),anchor:[x,y]};
  }
  ...
  function updateManualPlacementUi(){
    ...
    if(relicManualHint){
      relicManualHint.textContent=relicManualPlacementMode
        ?"手動配置ON：◎は形状の左上支点。PCはカーソルに追従、スマホはタップでプレビュー → 90°回転 → 「変更確定」で保存します。"
      ...
      relicV3Board.querySelectorAll("[data-index]").forEach(cell=>{
        cell.addEventListener("pointerenter",event=>{
          ...
          relicMessage.textContent=preview?.valid
            ?"配置可能です。カーソルに形状が追従します。必要なら90°回転 → 「変更確定」。"
  ...
  function renderRelicBoard() {
    ...
    relicPlacementList.querySelectorAll("[data-relic-manual]").forEach(button=>{
      button.addEventListener("click",event=>{
        ...
        beginManualDraft(pl);
        const item=context.itemsById.get(String(pl.itemId));
        relicMessage.textContent=`${item?.["名前"]||"レリック"}を手動配置対象にしました。形状の左上支点◎を置きたい盤面マスに合わせてください。PCはカーソル追従、スマホはタップでプレビューし、「変更確定」で保存します。`;
        renderRelicBoard();
      });
    });
