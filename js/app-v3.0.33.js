...
(() => {
  ...
  function initializeSkillData() {
    ...
    // v3.0.33:
    // 「未分類」は数値効果が未確定でもスキル選択肢として表示する。
    // ただし、同一JOB・同名で確定カテゴリ版が存在する場合は未分類版を表示せず二重表示を防ぐ。
    // 合計能力への反映可否は従来どおり SKILL_EFFECT_MASTER の「ビルド反映」で判定する。
    const categorizedSkillKeys = new Set(
      data.skills
        .filter(skill => String(skill["カテゴリ"] || "").trim() !== "未分類")
        .filter(skill => !String(skill["カテゴリ"] || "").includes("ペットスキル"))
        .map(skill => `${String(skill["職業ID"] || "")}\u0000${String(skill["スキル名"] || skill["選択肢名"] || "").trim()}`)
    );
    data.skills.forEach(skill => {
      const skillId = String(skill["スキルID"] || "");
      if (String(skill["有効"] || "TRUE").toUpperCase() === "FALSE") return;
      const category = String(skill["カテゴリ"] || "").trim();
      const isUncategorized = category === "未分類";
      const duplicateKey = `${String(skill["職業ID"] || "")}\u0000${String(skill["スキル名"] || skill["選択肢名"] || "").trim()}`;
      if (isUncategorized && categorizedSkillKeys.has(duplicateKey)) return;
      const isPet = category.includes("ペットスキル");
      if (isPet) {
...
function collectTotalData() {
  ...
  function restoreRgWorkspace(){
    ...
    try{
      ...
      console.warn("RG作業内容の読込に失敗したため初期化します",error);
      try{localStorage.removeItem(RG_WORKSPACE_KEY);}catch(_){}
      return false;
    }
