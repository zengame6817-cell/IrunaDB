...
(() => {
  ...
  const context = {
    ...
    relicPatternByItemId: new Map(), relicPatternByName: new Map(), relicPatternSharedByName: new Map(),
  ...
  function relicPatternForItem(itemId) {
    ...
    return context.relicPatternByName.get(key) || context.relicPatternSharedByName.get(key) || [];
  }
  ...
  function updateRelicBoardAutoCounts(){
    ...
    if(b.mode!=="none"){
      ...
      const occupied=relicPlacementOccupiedSet();
      Object.entries(preset.cells).forEach(([key,color])=>{
        if(occupied.has(key)) counts[color]+=1;
      });
    }
  ...
  function renderRelicV3Board(){
    ...
    state.build.relicPlacements.forEach((pl,pi)=>{
      absoluteRelicCells(pl).forEach(([x,y])=>{
        if(x>=0&&x<7&&y>=0&&y<7) occupied.set(y*7+x,pi+1);
      });
    });
    [...relicV3Board.children].forEach((cell,i)=>{
      ...
      if(occupied.has(i)){
        ...
        cell.innerHTML+=`<strong>${occupied.get(i)}</strong>`;
      }
    });
  }
  ...
  function renderRelicBoard() {
    ...
    relicPlacementList.querySelectorAll("[data-relic-select]").forEach(button => {
      button.addEventListener("click", () => {
        ...
        relicMessage.textContent = `${item?.["名前"] || "レリック"}を選択しました。`;
        renderRelicBoard();
      });
    });
