...
(() => {
  ...
  function relicManualRotatedCells(placement){
    return rotateCells(relicPatternForPlacement(placement),Number(placement?.rotation)||0);
  }

  function relicManualAnchorCell(placement){
    const cells=relicManualRotatedCells(placement)
      .slice().sort((a,b)=>(a[1]-b[1])||(a[0]-b[0]));
    return cells[0] || [0,0];
  }

  function relicManualCandidateAt(index, placement=null){
    const pl=placement || state.build.relicPlacements.find(entry=>entry.uid===state.selectedRelicUid);
    const pattern=relicPatternForPlacement(pl);
    if(!pl || pl.enabled===false || !pattern.length) return null;
    const tapX=index%7, tapY=Math.floor(index/7);
    const [ax,ay]=relicManualAnchorCell(pl);
    const candidate={...pl,x:tapX-ax,y:tapY-ay};
    return {
      candidate,
      valid:canPlaceRelic(candidate,pl.uid),
      cells:absoluteRelicCells(candidate),
      anchor:[tapX,tapY]
    };
  }
  ...
  function placeSelectedRelicManually(index){
    const pl=state.build.relicPlacements.find(entry=>entry.uid===state.selectedRelicUid);
    if(!pl){
      relicMessage.textContent="先に右側のレリックで「手動配置」を押してください。";
      return false;
    }
    const preview=relicManualCandidateAt(index,pl);
    if(!preview || !preview.valid){
      relicMessage.textContent="この支点位置では配置できません。赤いプレビューを確認して別のマスを選んでください。";
      relicManualPreviewIndex=index;
      renderRelicV3Board();
      return false;
    }
    pl.x=preview.candidate.x;
    pl.y=preview.candidate.y;
    relicManualPreviewIndex=null;
    relicV3Candidates=[];
    ...
    relicMessage.textContent=`${item?.["名前"]||"レリック"}を手動配置しました。◎がこのレリックの支点です。`;
    return true;
  }
  ...
  function updateManualPlacementUi(){
    ...
    if(relicManualHint){
      relicManualHint.textContent=relicManualPlacementMode
        ?"手動配置ON：◎が形状の支点です。盤面に支点を合わせ、1回タップでプレビュー → 同じマスをもう1回タップで確定します。"
        :"手動配置OFF：盤面タップは使用不可マスの切替です。";
    }
  }
      ...
      relicV3Board.querySelectorAll("[data-index]").forEach(cell=>{
        cell.addEventListener("click",()=>{
          ...
          if(relicManualPlacementMode){
            const hit=placementAtBoardIndex(i);
            if(hit && hit.uid!==state.selectedRelicUid){
              state.selectedRelicUid=hit.uid;
              relicManualPreviewIndex=null;
              const item=context.itemsById.get(String(hit.itemId));
              relicMessage.textContent=`${item?.["名前"]||"レリック"}を選択しました。◎の支点を置きたいマスを選んでください。`;
              renderBuild();
              return;
            }
            ...
            if(relicManualPreviewIndex!==i){
              ...
              relicMessage.textContent=preview?.valid
                ?"配置プレビュー中です。同じマスをもう一度タップすると確定します。"
                :"ここには配置できません。赤いプレビューを確認して別のマスを選んでください。";
              return;
            }
            placeSelectedRelicManually(i);
            return;
          }
          ...
          if(occupiedHit){
            ...
            relicMessage.textContent=`${item?.["名前"]||"レリック"}を選択しました。回転する場合は「選択を90°回転」を押してください。`;
            renderRelicV3Board();
            return;
          }
  ...
  function applyRelicV3Candidate(index,announce=true){
    ...
    if(firstPlaced) state.selectedRelicUid=firstPlaced.uid;
    relicManualPreviewIndex=null;
    updateRelicBoardAutoCounts();
    syncUrl(false);renderBuild();
...
function collectTotalData() {
  ...
  relicManualMode?.addEventListener("click",()=>{
    relicManualPlacementMode=!relicManualPlacementMode;
    relicManualPreviewIndex=null;
    updateManualPlacementUi();
    renderRelicV3Board();
  });
  relicRotateManual?.addEventListener("click",rotateSelectedRelic);
