...
(() => {
  ...
  function placeSelectedRelicManually(index){
    ...
    const candidate={...pl,x:index%7,y:Math.floor(index/7)};
    if(!canPlaceRelic(candidate,pl.uid)){
      relicMessage.textContent="そこには配置できません。盤面外・使用不可マス・他レリックとの重なりを確認してください。";
      return false;
    }
...
function collectTotalData() {
  ...
  function renderScreenshotSummary(totalData = collectTotalData()) {
    ...
    appliedRelicPlacements().forEach(entry => {
