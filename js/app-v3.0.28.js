...
(() => {
  ...
  function renderPetSkills() {
    ...
    const allRows = [...(context.petSkills || [])]
      .sort((x, y) => Number(x["表示順"] || 999) - Number(y["表示順"] || 999));
    if (!allRows.length) {
    ...
    // SKILL_MASTERには全JOB分の同一ペットスキルがあるため、表示時は内容で重複除去する。
    ...
    const cardFor = (name, rows) => {
      ...
      const desc = String(chosen?.["説明文"] || chosen?.["特殊効果"] || "");
      const safe = encodeURIComponent(name);
      const options = rows.map(row => {
        const id = String(row["スキルID"] || "");
        const label = String(row["選択肢名"] || row["適用Lv"] || name);
        return `<option value="${escapeHtml(id)}" ${String(chosen?.["スキルID"] || "") === id ? "selected" : ""}>${escapeHtml(label)}</option>`;
      }).join("");
      return `<article class="skill-card ${isOn ? "is-active" : ""}">
        <div class="skill-card-main"><label class="skill-toggle-row">
          <input type="checkbox" data-pet-toggle="${escapeHtml(safe)}" ${isOn ? "checked" : ""}>
          <span class="skill-card-copy"><b>${escapeHtml(name)}</b>${desc ? `<small>${escapeHtml(desc)}</small>` : ""}</span>
        </label></div>
        <label class="skill-level-control"><span>Lv・匹数</span><select data-pet-choice="${escapeHtml(safe)}" ${isOn ? "" : "disabled"}>${options}</select></label>
      </article>`;
    };

    petSkillSelections.innerHTML = `<div class="pet-skill-capacity ${total >= 18 ? "is-full" : ""}"><b>ペット編成</b><span>${total} / 18匹</span><small>ステータス系は匹数分加算。通常スキルはLv1/2/3で1/2/3匹を使用します。</small></div>` +
      `<section class="skill-group"><div class="skill-card-list">${[...byName].map(([name, rows]) => cardFor(name, rows)).join("")}</div></section>`;
    ...
    petSkillSelections.querySelectorAll('[data-pet-toggle]').forEach(input => input.addEventListener('change', () => {
      const rows = rowsFor(input.dataset.petToggle); clearRows(rows);
      if (input.checked) {
        const select = petSkillSelections.querySelector(`[data-pet-choice="${CSS.escape(String(input.dataset.petToggle || ""))}"]`);
        const id = String(select?.value || defaultRow(rows)?.["スキルID"] || "");
        if (id) state.selectedSkills.add(id);
        if (currentCount() > 18) { state.selectedSkills.delete(id); alert("ペットは最大18匹までです。"); }
      }
      finish();
    }));
    petSkillSelections.querySelectorAll('[data-pet-choice]').forEach(select => select.addEventListener('change', () => {
      const rows=rowsFor(select.dataset.petChoice); const previous=selectedRow(rows); clearRows(rows);
      const toggle=petSkillSelections.querySelector(`[data-pet-toggle="${CSS.escape(String(select.dataset.petChoice || ""))}"]`);
      if (toggle?.checked) {
        const id=String(select.value || ""); if(id) state.selectedSkills.add(id);
        if(currentCount()>18){ state.selectedSkills.delete(id); if(previous) state.selectedSkills.add(String(previous["スキルID"]||"")); alert("ペットは最大18匹までです。合計18匹以内にしてください。"); }
      }
      finish();
    }));
  }
