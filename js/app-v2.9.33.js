...
(() => {
  ...
  const createEmptyBuild = () => ({
    ...
    relicPlacements: [],
    relicBoard: { mode:"none", red:0, yellow:0, white:0, blue:0, green:0, black:0 }
  });
  ...
  const relicBoardSelects = {
    red:document.getElementById("relicBoardRed"), yellow:document.getElementById("relicBoardYellow"),
    white:document.getElementById("relicBoardWhite"), blue:document.getElementById("relicBoardBlue"),
    green:document.getElementById("relicBoardGreen"), black:document.getElementById("relicBoardBlack")
  };
  ...
  function clampRelicBoardCount(key,value){
  ...
  function collectRelicBoardBonuses(){
    ...
    const mode=["physical","magic"].includes(b.mode) ? b.mode : "none";
    const rows=[];
    const push=(statId,unit,value,label)=>{
      if(!value) return;
      rows.push({statId,unit,value,label,effectText:`${label} / レリック盤面`});
    };
    if(mode==="physical"){
      const red=clampRelicBoardCount("red",b.red), yellow=clampRelicBoardCount("yellow",b.yellow), white=clampRelicBoardCount("white",b.white);
      push("ATK","",red*20,`赤 ${red}マス`);
      push("ASPD","%",yellow*2,`黄 ${yellow}マス`);
      push("VIT","",white,`白 ${white}マス`);
    } else if(mode==="magic"){
      const blue=clampRelicBoardCount("blue",b.blue), green=clampRelicBoardCount("green",b.green), black=clampRelicBoardCount("black",b.black);
      push("MATK","",blue*20,`青 ${blue}マス`);
      push("詠唱時間","%",green*-2,`緑 ${green}マス`);
      push("INT","",black,`黒 ${black}マス`);
    }
    return {mode,rows};
  }

  function renderRelicBoardBonus(){
    const b=state.build.relicBoard || (state.build.relicBoard={mode:"none",red:0,yellow:0,white:0,blue:0,green:0,black:0});
    relicBoardMode.value=["physical","magic"].includes(b.mode) ? b.mode : "none";
    relicBoardPhysical.hidden=relicBoardMode.value!=="physical";
    relicBoardMagic.hidden=relicBoardMode.value!=="magic";
    Object.entries(relicBoardSelects).forEach(([key,select])=>{
      const max=RELIC_BOARD_MAX[key];
      if(!select.options.length) select.innerHTML=Array.from({length:max+1},(_,i)=>`<option value="${i}">${i}マス</option>`).join("");
      select.value=String(clampRelicBoardCount(key,b[key]));
    });
...
function collectTotalData() {
  ...
  function restoreBuildFromUrl() {
    ...
    try {
      ...
      state.build.relicBoard = {
        ...
        red:clampRelicBoardCount("red",savedRelicBoard.red),
        yellow:clampRelicBoardCount("yellow",savedRelicBoard.yellow),
        white:clampRelicBoardCount("white",savedRelicBoard.white),
        blue:clampRelicBoardCount("blue",savedRelicBoard.blue),
        green:clampRelicBoardCount("green",savedRelicBoard.green),
        black:clampRelicBoardCount("black",savedRelicBoard.black)
      };
  ...
  relicBoardMode.addEventListener("change",()=>{
    state.build.relicBoard.mode=relicBoardMode.value;
    syncUrl(false);
    renderBuild();
  });
  Object.entries(relicBoardSelects).forEach(([key,select])=>{
    select.addEventListener("change",()=>{
      state.build.relicBoard[key]=clampRelicBoardCount(key,select.value);
      syncUrl(false);
      renderBuild();
    });
  });
