...
(() => {
  ...
  function setView(view) {
    const targetView = view === "relic" ? "build" : view;
    ...
    document.querySelectorAll(".app-view").forEach(section => section.classList.toggle("is-active", section.id === `view-${targetView}`));
    ...
    if (view === "relic") {
      // v3.0.1: top-level「レリック」タブから既存のレリック欄へ直接移動。
      // DOMを複製しないので、選択中レリック・7×7配置・盤面ボーナスは完全に同じデータを使う。
      requestAnimationFrame(() => {
        const relicTarget = document.querySelector(".relic-board-bonus")?.closest(".sim-panel")
          || document.querySelector(".relic-board-bonus")
          || relicPlacementList;
        relicTarget?.scrollIntoView({ behavior: "smooth", block: "start" });
      });
    } else {
  ...
  function renderRelicBoardBonus(){
    ...
    relicV3SelectedSummary.innerHTML=state.build.relicPlacements.length
      ? `<strong>${state.build.relicPlacements.length}個選択中</strong>${missing.length?`<small> / 形状未登録 ${missing.length}個</small>`:""}`
      : "レリック未選択";
  ...
  function applyRelicV3Candidate(index,announce=true){
    ...
    updateRelicBoardAutoCounts();
    syncUrl(false);renderBuild();
    if(announce) relicV3SolveStatus.textContent=`候補 ${relicV3CandidateIndex+1} / ${relicV3Candidates.length} を反映しました。`;
  }
...
function collectTotalData() {
  ...
  relicV3ResetResult.addEventListener("click",()=>{
    ...
    relicV3SolveStatus.textContent="配置結果を消しました。";
  });
