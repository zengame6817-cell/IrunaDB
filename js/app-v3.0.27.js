...
(() => {
  ...
  function renderSkills() {
    ...
    // v3.0.27: ペットスキルは連れて行くペット最大18匹の枠を共有する。
    // ステータス系は「n匹」、通常スキルは選択Lvを必要匹数(1～3匹)として数える。
    const isPetSkill = row => String(row?.["カテゴリ"] || "").includes("ペットスキル");
    const petCountForRow = row => {
      if (!row || !isPetSkill(row)) return 0;
    ...
    const selectedPetCount = () => list.reduce((sum, row) => state.selectedSkills.has(String(row["スキルID"] || "")) ? sum + petCountForRow(row) : sum, 0);
    ...
    const petCount = selectedPetCount();
    const hasPetSkills = list.some(isPetSkill);
    skillSelections.innerHTML = `${hasPetSkills ? `<div class="pet-skill-capacity ${petCount >= 18 ? "is-full" : ""}"><b>ペット編成</b><span>${petCount} / 18匹</span><small>同一スキルは選択Lv分、ステータス系は選択匹数分を使用します。</small></div>` : ""}` + [...categories].map(([category, names]) =>
    ...
    skillSelections.querySelectorAll('[data-skill-toggle]').forEach(input => input.addEventListener('change', () => {
      ...
      if (input.checked) {
        ...
        if (selectedPetCount() > 18) {
          state.selectedSkills.delete(id);
          alert("ペットは最大18匹までです。現在の編成ではこのスキルを追加できません。");
        }
      }
      finishSkillChange();
    }));
    ...
    skillSelections.querySelectorAll('[data-skill-choice]').forEach(select => select.addEventListener('change', () => {
      ...
      if (toggle?.checked || rows.some(row => String(row["選択方式"] || "") === "AUTO")) {
        ...
        if (selectedPetCount() > 18) {
          state.selectedSkills.delete(id);
          if (previous) state.selectedSkills.add(String(previous["スキルID"] || ""));
          alert("ペットは最大18匹までです。合計18匹以内になるように調整してください。");
        }
      }
      finishSkillChange();
    }));
...
function collectTotalData() {
  ...
  function partialReset(kind){
    ...
    const labels={
      ...
      relic:"レリック",skills:"スキル",status:"ステータス",apostoria:"アポストリア"
    };
