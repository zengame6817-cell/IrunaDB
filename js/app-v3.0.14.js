...
(() => {
  ...
  function relicManualCandidateAt(index, placement=null, rotationOverride=null){
    ...
    const [ax,ay]=relicManualAnchorCell(pl,rotation);
    const candidate={...pl,x:tapX-ax,y:tapY-ay,rotation};
    const check=relicPlacementCheck(candidate,pl.uid);
    return {candidate,valid:check.valid,reason:check.reason,cells:absoluteRelicCells(candidate),anchor:[tapX,tapY]};
  }
  ...
  function selectedRelicActualAnchorIndex(){
    ...
    const x=Number(pl.x)+ax,y=Number(pl.y)+ay;
  ...
  function confirmManualPlacement(){
    ...
    if(relicManualPreviewIndex===null){ relicMessage.textContent="盤面をタップして配置位置を選んでください。"; return false; }
    const preview=relicManualCandidateAt(relicManualPreviewIndex,pl,relicManualDraftRotation);
    if(!preview?.valid){
      relicMessage.textContent=`変更できません：${preview?.reason||"配置不可"}。赤いプレビューを確認してください。`;
      renderRelicV3Board();
      return false;
    }
    pl.x=preview.candidate.x;pl.y=preview.candidate.y;pl.rotation=preview.candidate.rotation;
    relicV3Candidates=[];relicV3CandidateIndex=0;
  ...
  function cancelManualDraft(){
    ...
    relicMessage.textContent="変更を取り消しました。";
    renderRelicV3Board();
  }
  ...
  function updateManualPlacementUi(){
    ...
    if(relicManualConfirm) relicManualConfirm.disabled=!relicManualPlacementMode || relicManualPreviewIndex===null;
    if(relicManualCancel) relicManualCancel.disabled=!relicManualPlacementMode;
    if(relicManualHint){
  ...
  function renderRelicV3Board(){
    ...
    if(!relicV3Board.children.length){
      ...
      relicV3Board.querySelectorAll("[data-index]").forEach(cell=>{
        cell.addEventListener("click",()=>{
          ...
          if(relicManualPlacementMode){
            relicManualPreviewIndex=i;
            const preview=relicManualCandidateAt(i);
            updateManualPlacementUi();
            renderRelicV3Board();
            relicMessage.textContent=preview?.valid
              ?"配置可能です。必要なら90°回転して、「変更確定」を押してください。"
              :`ここは配置不可：${preview?.reason||"不明"}。赤いプレビューを確認してください。`;
            return;
          }
      ...
      relicV3Board.addEventListener("pointerover",event=>{
        if(!relicManualPlacementMode || event.pointerType==="touch") return;
        const cell=event.target.closest("[data-index]");
        if(!cell) return;
        const i=Number(cell.dataset.index);
        if(relicManualPreviewIndex!==i){
          relicManualPreviewIndex=i;
          renderRelicV3Board();
        }
      });
    ...
    const manualPreview=(relicManualPlacementMode && relicManualPreviewIndex!==null)
      ? relicManualCandidateAt(relicManualPreviewIndex) : null;
...
function collectTotalData() {
  ...
  relicRotateManual?.addEventListener("click",()=>{
    ...
    updateManualPlacementUi();renderRelicV3Board();
    const preview=relicManualPreviewIndex===null?null:relicManualCandidateAt(relicManualPreviewIndex,pl,relicManualDraftRotation);
    relicMessage.textContent=preview
      ?(preview.valid?"90°回転プレビューです。「変更確定」で保存します。":`90°回転後は配置不可：${preview.reason}。位置を変えてください。`)
  ...
  relicBlockEditMode?.addEventListener("click",()=>{
    ...
    if(relicBlockEditModeEnabled){
      relicManualPlacementMode=false;relicManualPreviewIndex=null;relicManualDraftRotation=null;
    }
    updateManualPlacementUi();renderRelicV3Board();
  });
