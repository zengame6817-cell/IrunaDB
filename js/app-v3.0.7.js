...
(() => {
  ...
  function allSelectedSourceEntries() {
    ...
    appliedRelicPlacements().forEach((entry, index) =>
  ...
  function loadSelectedRelicShapeIntoEditor(){
    ...
    const shape=relicPatternForItem(pl.itemId);
  ...
  function renderBuildRelicSummary(){
    if(!buildRelicAppliedList) return;
    const applied=appliedRelicPlacements();
    if(!applied.length){
      buildRelicAppliedList.innerHTML='<span class="build-relic-empty">配置中のレリックはありません。 <button type="button" data-open-rg-inline>RGで配置する</button></span>';
    }else{
      const counts=new Map();
      applied.forEach(pl=>{
        const item=context.itemsById.get(String(pl.itemId));
        const name=String(item?.["名前"]||pl.itemId);
        counts.set(name,(counts.get(name)||0)+1);
      });
      buildRelicAppliedList.innerHTML=[...counts.entries()].map(([name,count])=>`<span class="build-relic-chip"><b>${escapeHtml(name)}</b>${count>1?` ×${count}`:""}</span>`).join("");
    }
    buildRelicAppliedList.querySelector("[data-open-rg-inline]")?.addEventListener("click",()=>setView("relic"));
  }

  function renderBuild() {
    renderSkills();
    renderBuildRelicSummary();
