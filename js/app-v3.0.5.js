...
(() => {
  ...
  const context = {
    ...
    relicPatternByItemId: new Map(), relicPatternByName: new Map(), relicPatternSharedByName: new Map(), relicPatternLocalByItemId: new Map(),
  ...
  function isRelicApplied(entry) {
    if (!entry || !relicPatternForItem(entry.itemId).length) return false;
  ...
  function relicPatternForItem(itemId) {
    const id=String(itemId);
    const direct = context.relicPatternByItemId.get(id);
    if (direct) return direct;
    const item = context.itemsById.get(id);
    const key = normalizeSearchText(item?.["名前"] || "");
    return context.relicPatternByName.get(key)
      || context.relicPatternLocalByItemId.get(id)
      || context.relicPatternSharedByName.get(key)
      || [];
  }
    ...
    try {
      ...
      rows.forEach(row => {
        ...
        const shape = Array.isArray(row?.shape)
          ...
          : [];
        if (name && shape.length && !context.relicPatternSharedByName.has(name)) {
          context.relicPatternSharedByName.set(name, shape);
        }
      });
  ...
  function renderRelicBoardBonus(){
    ...
    renderRelicV3Board();
    const missing=state.build.relicPlacements.filter(pl=>!relicPatternForItem(pl.itemId).length);
  ...
  function relicV3Variants(itemId){
    const base=relicPatternForItem(itemId);
  ...
  function solveRelicV3(){
    const b=normalizeRelicBoardState();
    const pieces=state.build.relicPlacements.map(pl=>({
      uid:pl.uid,itemId:pl.itemId,name:context.itemsById.get(String(pl.itemId))?.["名前"]||"レリック",
      variants:relicV3Variants(pl.itemId),area:relicPatternForItem(pl.itemId).length
    }));
  ...
  function relicMiniShapeHtml(itemId){
    const shape=relicPatternForItem(itemId);
  ...
  function renderRelicBoard() {
      ...
      ? filtered.map(({placement,index,item}) => {
          ...
          return `<article class="relic-tool-piece ${selected ? "is-selected" : ""} ${applied ? "is-applied" : "is-candidate"}" data-placement-uid="${placement.uid}">
            ...
            <div class="relic-tool-piece-body">
              ...
              ${relicMiniShapeHtml(placement.itemId)}
            </div>
    ...
    relicPlacementList.querySelectorAll("[data-relic-select]").forEach(button => {
      button.addEventListener("click", () => {
        ...
        relicMessage.textContent = `${item?.["名前"] || "レリック"}を選択しました。色設定・形状編集ができます。`;
        renderRelicBoard();
      });
    });
    ...
    if(relicCandidateCells) relicCandidateCells.textContent=state.build.relicPlacements.reduce((s,x)=>s+relicPatternForItem(x.itemId).length,0);
