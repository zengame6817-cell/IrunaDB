...
(() => {
  ...
  function migrateLegacyRelicBlockedCellsV3016(){
    const key="irunadb.relic.blockedMigration.v3.0.17";
  ...
  function relicManualRotatedCells(placement,rotation=Number(placement?.rotation)||0){
    return rotateCells(relicPatternForPlacement(placement),rotation);
  }

  function relicManualAnchorCell(placement,rotation=Number(placement?.rotation)||0){
    const cells=relicManualRotatedCells(placement,rotation)
      .slice().sort((a,b)=>(a[1]-b[1])||(a[0]-b[0]));
    return cells[0] || [0,0];
  }
  ...
  function updateManualPlacementUi(){
    ...
    if(relicManualHint){
      relicManualHint.textContent=relicManualPlacementMode
        ?"手動配置ON：◎はピース内の中心支点。PCはカーソルに追従、スマホはタップでプレビュー → 90°回転 → 「変更確定」で保存します。"
    ...
    if(!relicV3Board.children.length){
      ...
      relicV3Board.querySelectorAll("[data-index]").forEach(cell=>{
        cell.addEventListener("pointerenter",event=>{
          if(!relicManualPlacementMode || event.pointerType==="touch") return;
        ...
        cell.addEventListener("click",()=>{
          ...
          if(relicManualPlacementMode){
            ...
            updateManualPlacementUi();renderRelicV3Board();
            relicMessage.textContent=preview?.valid
              ?"配置可能です。緑の形を確認し、必要なら90°回転 → 「変更確定」を押してください。"
              :`ここは配置不可：${preview?.reason||"不明"}。別のマスを選んでください。`;
            return;
          }
          const occupiedHit=placementAtBoardIndex(i);
          if(occupiedHit){
            state.selectedRelicUid=occupiedHit.uid;
            relicManualDraftRotation=Number(occupiedHit.rotation)||0;
            relicManualPreviewIndex=null;
            relicManualDraftCandidate=null;
            const item=context.itemsById.get(String(occupiedHit.itemId));
            relicMessage.textContent=`${item?.["名前"]||"レリック"}を選択しました。「手動配置 ON」にすると位置・向きを変更できます。`;
            renderRelicV3Board();
            return;
          }
...
function collectTotalData() {
  ...
  relicBlockEditMode?.addEventListener("click",()=>{
    ...
    if(relicBlockEditModeEnabled){
      relicManualPlacementMode=false;relicManualPreviewIndex=null;relicManualDraftRotation=null;relicManualDraftCandidate=null;
    }
    updateManualPlacementUi();renderRelicV3Board();
  });
