...
window.IrunaUi = (() => {
  ...
  function renderItems(items, context, onOpen) {
    ...
    elements.itemGrid.innerHTML = items.map((item, index) => `<article class="item-card" data-item-index="${index}" tabindex="0"><div class="item-card-header"><div class="item-name">${escapeHtml(item["名前"] || "名称未設定")}</div><span class="badge">${escapeHtml(item["表示分類"] || item["サブ分類"] || item["分類"] || "未分類")}</span></div><div class="item-meta">${buildMeta(item, context.attributeMap)}</div><div class="item-description">${escapeHtml(item["説明文"] || item["特殊性能"] || "説明はまだ登録されていません。")}</div></article>`).join("");
